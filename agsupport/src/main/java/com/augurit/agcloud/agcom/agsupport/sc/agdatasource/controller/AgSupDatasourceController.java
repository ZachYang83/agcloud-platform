...
public class AgSupDatasourceController {
    ...
    public ResultForm save(AgSupDatasource datasource, HttpServletRequest request) throws Exception {
        if(StringUtils.isNotBlank(datasource.getId())){
            ...
            if ("oracle".equals(dbType.toLowerCase())){
                ...
                datasource.setDbUrl("jdbc:mysql://"+datasource.getIp()+":"+datasource.getPort()+"/"+datasource.getSid());
            }else if ("dm".equals(dbType.toLowerCase())){
                datasource.setDbUrl("jdbc:dm://"+datasource.getIp()+":"+datasource.getPort());
            }
            //修改了数据源密码时，需要对密码加密
            AgSupDatasource datasourcePwd = supDatasource.selectDataSourceById(datasource.getId());
    ...
    private void update(AgSupDatasource datasource) throws Exception{
        int i = supDatasource.updateDataSource(datasource);
    }

    @SysLog(sysName = "地图运维",funcName = "删除数据源")
    ...
    public ResultForm testConnection(String dbType,String userName,String password,String dbUrl){
        ...
        }else if (DbConstants.DM.equals(dbType)){
            driverClassname = DbConstants.DM_DRIVER;
        }
        try {
