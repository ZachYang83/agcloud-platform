...
//                if(publisher.publishShp("luogang", "sfdem", "zm", zipFile, "EPSG:4326")){
    ...
    private static Map<String,JSONArray> mapCache=new HashMap<>();
    ...
    public ContentResultForm getReports(String page, String rows, String serverId) throws Exception {
        try {
            ...
            String serverType = agServer.getType();
            ...
            if(!Common.isCheckNull(port)){
                urlString = urlString+":" + port;
            }
            urlString = urlString+ "/arcgis/rest/services?f=pjson";
            JSONArray jsonArray = new JSONArray();
            if (!Common.isCheckNull(values)||mapCache.containsKey(key)) {
                //jsonArray = JSONArray.fromObject(values);
                jsonArray = mapCache.get(key);
            } else {
    ...
    private void getListService(URL url,JSONArray jsonArraay) throws Exception{
        ...
        for (int i = 0; i < servicesJsonArray.size(); i++) {
            try {
                ...
                if (!Common.isCheckNull(supportedExtensionsType)) {
...
//            Json1.put("alias", alias);
   ...
   /* @RequestMapping("/deleteser")
    ...
    public JSONObject getcolum1(@PathVariable("ip") String ip, @PathVariable("name") String name, @PathVariable("port") String port, @PathVariable("type") String type, @PathVariable("fold") String fold,String serverId) throws Exception {
        String serviceName =fold +"/"+ name.replace(fold+"/","");
        String token = null;
        String userName =null;
        String password = null;
        if(!Common.isCheckNull(serverId)) {
            AgServer agServer = iAgServer.selectServerById(serverId);
            userName = agServer.getUserName();
            password = agServer.getPassword();
            password = DESedeUtil.desDecrypt(password);
            token = agServer.getToken();
        }
        String url = "http://" + ip ;
        if(!Common.isCheckNull(port)){
            url = url+":" + port;
        }
        url = url+ "/arcgis/rest/services/" + serviceName + "/MapServer?f=pjson";
        if(!Common.isCheckNull(token)){
            url += "&token="+token;
        }else if(!Common.isCheckNull(userName)&&!Common.isCheckNull(password)){
            token = loginin(userName, password, ip, port);
    ...
    public JSONArray getcolum(@PathVariable("ip") String ip, @PathVariable("name") String name, @PathVariable("port") String port, @PathVariable("type") String type, @PathVariable("fold") String fold,int parentId,int len,String serverId) throws Exception {
        String serviceName = fold + "/" + name.replace(fold + "/", "");
        String token = null;
        String userName =null;
        String password = null;
        ...
        for (int i = 0; i < layers.size(); i++) {
            ...
            String url1 = url.substring(0,url.indexOf("?")) + "/" + i + "?f=pjson";
            if(!Common.isCheckNull(token)) {
                url1 += "&token=" + token;
            }
            String result2 = defaultHttpClientUtil.doGet(url1, charset);
...
//            }
    ...
    public ContentResultForm getSelLayer(String serverId, String reip, String restport, String sellayer) throws Exception {
        String a[] = sellayer.split(",");
        ...
        if (a.length > 0) {
            for (int i = 0; i < a.length / 3; i++) {
                ...
                feilds = getcolum(reip, a[3 * i], restport, a[3 * i + 1], a[3 * i + 2],parentId,len+i,serverId);
                if (feilds.size() > 0) {
                    JSONObject Json1 = new JSONObject();
                    ...
                    JSONObject object = getcolum1(reip, a[3 * i], restport, a[3 * i + 1], a[3 * i + 2],serverId);
        ...
        return new ContentResultForm<JSONArray>(true,feilds1,"服务获取成功");
    }
