...
public class AgUserImpl implements IAgUser {
    ...
    public PageInfo<AgUser> findUsersByRoleIds(String roleIds,String userName, Page page) {
        ...
        if (StringUtils.isNotBlank(roleIds)) {
            ...
            if(!Common.isCheckNull(userName)){
                agUserList = agUserList.stream().filter(u->(u.getUserName()!=null&&u.getUserName().contains(userName))
                        ||(u.getLoginName()!=null&&u.getLoginName().contains(userName))).collect(Collectors.toList());
            }
            pageInfo.setTotal(Long.valueOf(map.get(TOTAL).toString()));
    ...
    public List<Map> getRolesByUserName(String userName){
        ...
        if (StringUtils.isNotBlank(userName)) {
            ...
            String result = HttpClientUtil.getByToken(url, param, token, "utf-8");
            try {
                listMap = com.augurit.agcloud.framework.util.JsonUtils.parseList(Map.class, result);
            }catch (Exception e){
                LOGGER.error("用户角色数据转换异常",e.getMessage());
            }
...
//        }
    ...
    public PageInfo<AgUser> searchUser(AgUser agUser, Page page, String orgId) throws Exception {
        ...
        if (agcloudInfLoad.booleanValue()) {
            ...
            if (StringUtils.isNotBlank(result)){
                Map<String, Object> m = com.augurit.agcloud.framework.util.JsonUtils.mapFromJson(result);
                List<Map> listMap = (List<Map>) m.get("rows");
                pageInfo.setTotal(Long.valueOf(m.get(TOTAL).toString()));
    ...
    @Override
    public List<AgUserLayer> findListByUserIdNotWithExtent(String userId) throws Exception {
        List<AgUserLayer> list = agUserLayerMapper.findListByUserIdNotWithExtent(userId);
        return list;
    }
...
//                    keys.add("access#role.user_role.role_layer:" + agUser.getId() + roleLayer.getDirLayerId());
            ...
            Set<String> project_info = stringRedisTemplate.keys("user#project_info:*");
            stringRedisTemplate.delete(userProject);
            stringRedisTemplate.delete(project_info);
    ...
    public PageInfo<AgUser> searchUserByLayer(AgUser agUser, String dirLayerId, Page page) throws Exception {
        ...
        if (!agcloudInfLoad.booleanValue()) {
            ...
            userPage.setTotal(Long.valueOf(m.get("total").toString()));
        }
...
//                    keys.add("access#role.user_role.role_layer:" + agUser.getId() + dirLayerId);
    ...
    @Override
    public List<AgUser> findAuthorizedUsers(List<AgDirLayer> agDirLayers) throws Exception {
        List<AgUserLayer> agUserLayerList = agUserLayerMapper.findByDirLayerIds(agDirLayers);
        List<String> userIdsList = agUserLayerList.stream().map(a -> a.getUserId()).collect(Collectors.toList());
        // 去重复userIds
        List<String> userIds = userIdsList.stream().distinct().collect(Collectors.toList());
        String ids = "";
        for (String id :  userIds){
            ids = ids + id + ",";
        }
        ids = ids.substring(0,ids.length()-1);
        List<AgUser> agUsers = findUsersByUserIds(ids);
        return agUsers;
    }
    ...
    public void delDirAuthor(List<AgDirLayer> agDirLayers, String userIds) throws Exception {
        List<AgUserLayer> agUserLayerList = agUserLayerMapper.findByDirLayerIds(agDirLayers);
        String[] delUserIds = userIds.split(",");
        List<String> ids = new ArrayList();
        for (String userId : delUserIds){
            for (AgUserLayer agUserLayer : agUserLayerList){
                if (userId.equals(agUserLayer.getUserId())){
                    ids.add(agUserLayer.getId());
                }
            }
        }
        if (ids.size() > 0){
