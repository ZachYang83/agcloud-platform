...
/*    *//**
    ...
    *//**
    ...
    @ApiOperation(value = "获取图层配置",notes = "获取图层配置")
    @ApiImplicitParams({
    ...
    public ContentResultForm getLayerConfig(String userId, String dirLayerId) throws Exception {
        ...
        Map map = new HashMap();
        map.put("agUserLayer", agUserLayer);
        map.put("data", iAgDir.findLayerByDirLayerId(agUserLayer.getDirLayerId()).getData());
    ...
    public ResultForm saveLayerConfig(AgUserLayer agUserLayer, String data, HttpServletRequest request) {
        if(Common.isCheckNull(agUserLayer.getUserId())){
            return new ResultForm(false,"userid不能为空");
        }
        JSONObject jsonObject = this.getLogInfo(request, "修改图层配置");

        try {
            String userIds = agUserLayer.getUserId();
            String dirLayerIds = agUserLayer.getDirLayerId();
...
//            iAgDir.updateLayer(agLayer);
    ...
    public ContentResultForm getLayerFieldConf(String dirLayerId,String layerId) throws Exception {
        Map map = new HashMap();
        map.put("agUserLayer", iAgField.getLayerFields(dirLayerId, layerId));
        map.put("data", iAgDir.findLayerByDirLayerId(dirLayerId).getData());
        return new ContentResultForm(true,map,"获取字段配置");
    }
