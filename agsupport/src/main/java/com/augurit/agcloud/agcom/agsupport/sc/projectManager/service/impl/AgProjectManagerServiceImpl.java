...
@Service
@Transactional
public class AgProjectManagerServiceImpl implements IAgProjectManagerService {
     ...
     */
    public void save(AgProjectManager agProjectManager, HttpServletRequest request) throws Exception {

        if (StringUtils.isNotEmpty(agProjectManager.getId())) {
            // 查询名称是否已存在
            AgProjectManager pm = projectManagerMapper.findByName(agProjectManager.getName());
            if (pm != null && !pm.getId().equals(agProjectManager.getId())){
                throw  new Exception("工程名称已存在");
            }
            // 修改时间
