...
public class AgDirController {
     ...
     * @return
     */
    @SysLog(sysName = "地图运维", funcName = "修改图层目录名称")
...
//            agDir.setMapParamId(mapParamId); 取消参数设置
    ...
    @SysLog(sysName = "地图运维", funcName = "删除图层目录")
    ...
    public ContentResultForm layersByXpath(HttpServletRequest request, @PathVariable("dirId") String dirId, String isBaseMap, String isContain, AgLayer agLayer, Page page) throws Exception {
        ...
        for (AgLayer eachLayer : list) {
            eachLayer.setDirPath(eachLayer.getDirPath().replace("/目录管理", ""));
        }
        return new ContentResultForm(true, PageHelper.toEasyuiPageInfo(pageInfo));
    }

    @ApiOperation(value = "数据缓冲获取指定目录包含的图层", notes = "数据缓冲获取指定目录包含的图层")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "dirId", value = "目录ID", paramType = "path", required = true),
            @ApiImplicitParam(name = "isBaseMap", value = "是否仅底图,1:是 0:否 ", dataType = "String"),
            @ApiImplicitParam(name = "isContain", value = "是否包含子节点,1:是 0:否", dataType = "String"),
     ...
     * @return
     */
    @SysLog(sysName = "地图运维", funcName = "删除图层")
     ...
     * @return
     */
    @SysLog(sysName = "地图运维", funcName = "移除目录的图层")
    ...
    @SysLog(sysName = "地图运维", funcName = "图层目录拖拽排序")
...
//            }
    ...
    @SysLog(sysName = "地图运维", funcName = "图层拖拽移动")
...
//            }
     ...
     * @return
     */
    @SysLog(sysName = "地图运维", funcName = "目录树添加图层")
 ...
 /*   @RequestMapping("/getChildLayers")
    ...
    @ApiImplicitParam(name = "key", value = "获取代理的前缀")
    ...
    @ApiOperation(value = "获取代理地址", notes = "获取代理地址接口（3dtlies）")
    ...
    public ContentResultForm<String> getProxyUrl2(String layerType, HttpServletRequest request) {
        try {
            // 3dtiles 代理格式：  http://ip:port/proxy/rest/services/userid/layerID/layerType
            // 获取登陆名称
            String loginName = LoginHelpClient.getLoginName(request);
            AgUser agUser = iAgUser.findUserByName(loginName);
            // 获取用户id
            ...
            return new ContentResultForm<String>(false, "", "获取代理地址失败，用户未登录或不存在！");
        }
    }
   ...
   /* @RequestMapping("/layerPreview")
    ...
    public ResultForm saveLayer(AgLayer agLayer, HttpServletRequest request) {
        ...
        try {
            ...
            e.printStackTrace();
            result = new ResultForm(false, "发生异常：" + e.getMessage());
        }
        return result;
    }
    ...
    private ResultForm addLayer(AgLayer agLayer, HttpServletRequest request) throws Exception {
        ...
        if (temp == null) {
            if (!iAgDir.existSameNameLayer(agLayer)) {
                ...
                return new ResultForm(true, "新增图层成功");
            } else {
                return new ResultForm(false, "已存在相同名称的图层");
            }
        } else {
            return new ResultForm(false, "服务已注册");
        }
    }
    ...
    private ResultForm updateLayer(AgLayer agLayer, HttpServletRequest request) throws Exception {
        ...
        if (temp == null || agLayer.getId().equals(temp.getId())) {
            if (!iAgDir.existSameNameLayer(agLayer)) {
                ...
                return new ResultForm(true, "修改图层成功");
            } else {
                return new ResultForm(false, "已存在相同名称的图层");
            }
        } else {
    ...
    private void setLayer(AgLayer agLayer, HttpServletRequest request) throws Exception {
        ...
        /**
         * date:2020.7.22
         * des： 添加三维代理url判断, 必须先新增保存图层才能代理
         * "120010" : 3dtiles 图层类型的编码
         */
        String proxyUrl = "";
    ...
    private String getProxyUrl2(AgLayer agLayer, AgUser agUser) throws Exception {
        // 获取用户id
        String userId = agUser.getId();
        // 三维代理url格式：http://ip:port/proxy/rest/services/userid/layerID/layerType
