...
//为了跟agproxy的检验缓存一致，使用uuid做缓存id
    ...
    public PageInfo<AgServiceLog> searchAgServiceLog(AgServiceLog agServiceLog, Page page) {
        PageHelper.startPage(page);
        //  日期范围查询应从起始时间的00:00:00至截止时间的23:59:59
        if (!Common.isCheckNull(agServiceLog.getAccessTimeEnd())) {
            Calendar cal = Calendar.getInstance();
            cal.setTime(agServiceLog.getAccessTimeEnd());
            cal.add(Calendar.DAY_OF_YEAR, 1);
            agServiceLog.setAccessTimeEnd(cal.getTime());
        }
        List<AgServiceLog> agServiceLogs;
        if (!agcloudInfLoad.booleanValue()) {
            agServiceLogs = agServiceMapper.listAgServiceLog(agServiceLog);//从数据库中获取
            return new PageInfo<AgServiceLog>(agServiceLogs);
        } else {
            try {
                List<AgUser> listUser = iAgUser.searchUsers(agServiceLog.getUserName());
                List<String> listIds = listUser.stream().map(u->u.getId()).collect(Collectors.toList());
