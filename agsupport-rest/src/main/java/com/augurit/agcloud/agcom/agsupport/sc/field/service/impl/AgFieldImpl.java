...
//        List<AgLayerFieldConf> layerFields = agLayerFieldConfMapper.findByDirLayerId(dirLayerId);
        ...
        if (layerFields.size() == 0) {
            ...
            if (agLayer.getLayerType().startsWith("04")) {//WFS图层判断
                try {
                    ...
                    LOGGER.error("wfs服务:"+layerFields.size());
                } catch (Exception e) {
    ...
    @Override
    public List<AgLayerRelated> getLayerRelatedByServiceDirLayerId(String serviceDirLayerId) throws Exception{
         return agLayerRelatedMapper.getByServiceDirLayerId(serviceDirLayerId);
    }
    ...
    private List<AgLayerFieldConf> getVectorFieldInfo(AgLayer agLayer) throws Exception {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (field_name == null){
                field_name = String.valueOf(map.get("FIELD_NAME"));
            }

            Object field_type = map.get("field_type");
            if (field_type == null){
                field_type = String.valueOf(map.get("FIELD_TYPE"));
            }
            Object field_size = map.get("field_size");
            if (field_size == field_size){
                field_size = String.valueOf(map.get("FIELD_SIZE"));
            }
...
//                }
    ...
    public void saveLayerConfig(AgUserLayer agUserLayer) {
        try {
            String _userLayerId = iAgUser.getUserLayer(agUserLayer.getUserId(), agUserLayer.getDirLayerId()).getId();
            agUserLayer.setId(_userLayerId);
            ...
            agUserLayer.setEditable(editable);
            agUserLayer.setQueryable(queryable);
            agUserLayer.setIsShow(isShow);
            agUserLayer.setIsBaseMap(isBaseMap);
            iAgUser.saveUserLayer(agUserLayer);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
